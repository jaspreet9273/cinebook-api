...
class KafkaProducer {
  ...
  async connect(): Promise<void> {
    ...
    this.producer = this.kafka.producer({
      idempotent: true,
      maxInFlightRequests: 5, // max allowed with idempotent=true
    });
   ...
   * Message key = showId or bookingId — ensures ordering within a show's partition.
   */
  async publish(topic: string, event: KafkaEvent): Promise<void> {
    ...
    // Partition key: prefer showId for seat ordering, fall back to bookingId
    const partitionKey =
      (event.payload.showId as string) ??
